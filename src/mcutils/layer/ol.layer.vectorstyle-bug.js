...
import ol_style_Style from 'ol/style/Fill'
 ...
 */
ol_layer_VectorStyle.clusterImage = function(options) {
  options = options || {};
  ...
  return function(f, res, clustered) {
    ...
    if (f.get('features')) {
        ...
        if (cluster.length == 1) {
          ...
          var styleid = 'cluster:'+size+'-'+(options.clusterColor||'')+'-'+(options.clusterDash===false?0:1);
          style = self._cacheStyle[styleid];
          if (!style) {
            style = self._cacheStyle[styleid] = new ol_style_Style({
              image: ol_layer_VectorStyle.clusterImage({size:size, color:clusterColor}),
      ...
      if (!(st = self._cacheStyle[id.main])) {
        ...
        if (clustered && typeGeom!=='Point') {
          img = ol_layer_VectorStyle.clusterImage({size:1, color:clusterColor});
        } else if (s.pointIcon) {
          var icon = /^https?:\/\//.test(s.pointIcon) ? s.pointIcon : md2html.server+'image/voir/'+s.pointIcon;
      ...
      if (s.pointShadow && typeGeom == 'Point') {
        ...
        if (!(st = self._cacheStyle[id.arrow])) {
          ...
          self._cacheStyle[id.arrow] = st = new ol_style_Style({
      ...
      if (label) {
        ...
        // Label avec affichaege conditionnel si le champ est vide
...
var ol_geom_getFlatCoordinates = function(coords) {
  if (coords && coords[0].length && coords[0][0].length) {
    ...
    for (var i=0; i<coords.length; i++) {
      c = c.concat(ol_geom_getFlatCoordinates(coords[i]));
    }
    return c;
  } else return coords;
};
...
*	- points {ol.style.Image | function | false | undefined} style for the points or false to hide points, default red points
...
var ol_style_getImage4Style = function(style, options) {
  ...
  switch (options.type) {
    case 'Text':
      ...
      ctx.restore();
      geom = new ol_geom_Point([w/2, h/2]);
    ...
    case 'Point':
      ...
      geom = new ol_geom_Point([w/2+offset[0], h/2+offset[1]]);
      if (options.calcStyleGeom) style = options.calcStyleGeom(geom);
      drawStyle(style, geom);
      break;
    case 'LineString':
      dl = options.padding||0;
      h = canvas.height = getStrokeWidth(style) + 2*dl;
      geom = new ol_geom_LineString([[0, h/2], [w-dl-2, h/2]]);
      if (options.calcStyleGeom) style = options.calcStyleGeom(geom);
      drawStyle(style, geom);
      break;
    case 'Polygon':
      dl = getStrokeWidth(style)/2;
      geom = new ol_geom_Polygon([[[dl, dl], [w-dl, dl], [w-dl, h-dl], [dl, h-dl], [dl, dl]]]);
      if (options.calcStyleGeom) style = options.calcStyleGeom(geom);
      drawStyle(style, geom);
      break;
    default:
...
/* Patch ol/source/Cluster to activate/daactivate cluster calculation
*/
(function(){
   var c = ol.source.Cluster.prototype.cluster
   ol.source.Cluster.prototype.cluster = function() {
      if (this.get('inactive')) return;
      c.call(this);
   }

   var r = ol.source.Cluster.prototype.refresh;
...
* @param {} options
*/
ol.layer.VectorStyle.prototype.setType = function(type, options) {
  ...
  switch (type) {
    case 'image':{
      if (!this.layerImage_) {
        this.layerImage_ = new ol.layer.Vector(
          {name: 'image',
    ...
    case 'cluster':{
      if (!this.layerCluster_ ) {
        var clusterSource = new ol.source.Cluster({
          geometryFunction: function(f) {
              ...
              else return new ol.geom.Point(ol.extent.getCenter(g.getExtent()));
          },
          distance: 40,
          source: this.getSource(),
        });
        this.layerCluster_ = new ol.layer.AnimatedCluster(
          {name: 'cluster',
              displayInLayerSwitcher: false,
      ...
      this.layerCluster_.setStyle(ol.layer.VectorStyle.getStyleFn(options));
...
/** Activer le calcul des clusters
* @param {boolean} b
*/
ol.layer.VectorStyle.prototype.activateCluster = function(b) {
   if (this.layerCluster_) {
      this.layerCluster_.getSource().set('inactive', !b);
      if (b) ol.source.Vector.prototype.refresh.call(this.layerCluster_.getSource());
   }
};
...
*/
ol_Feature.prototype.getLayer= function() {
  return this._layer;
};
...
(function() {
  ...
  */
  var formatLength = function(line) {
    ...
    */
    // N'est disponible qu'à partir de la version 5
    if(!ol.sphere) return;
  ...
  */
  ...
  ol_Feature.prototype.getPopupContent = function(content) {
   ...
   // if (!format) return '';
    ...
    var container = $('<div>');
    var myMd = $('<div>').addClass('md');
    myMd.html(md);
    myMd.appendTo(container);
...
ol_Feature.prototype.showPopup = function(popup, coord, geom) {
  ...
  // Charger le contenu twitter
  if (/twitter/.test(content)) {
    if (window.twttr) {
      twttr.widgets.load(popup.getElement());
    } else {
...
export {ol_style_getImage4Style}
export {ol_style_getImage4IgnStyle}
