...
*/
ol.source.Geoportail.prototype.attribution = new ol.control.Attribution
...
*/
ol.Map.Geoportail = function(options) {	// constructor
  ...
  this.listener_moveend = this.on('moveend', this.setLayerAttributions.bind(this));
...
// Get default attribution
   function getAttrib(map, mode, a, o) {
      ...
      if (!a) {
         return ol.control.Attribution.uniqueAttributionKey['IGN'+islogo] || ol.control.Attribution.getUniqueAttribution();
      }
      // Create attribution (if not exist)
      var attr;
      if (!ol.control.Attribution.uniqueAttributionKey[a+islogo]) {
         attr = map.formatAttribution(a, o.attribution||a, o.href, islogo ? o.logo:null);
      }
      // Create attribution
      return ( ol.control.Attribution.getUniqueAttribution(attr, a+islogo ));
   }

   // Set attribution according to position / zoom
   function setLayerAttribution(map, l, ex, z, mode) {	// Geoportail layer
      if (l._originators) {
...
*/
ol.Map.Geoportail.prototype.addLayer = function(layer) {	// Recursive setkey for group layers
   function setLayerKey(layer, key, authentication) {	// Geoportail layer
...
/**
 * Static function : ol.control.Attribution.getUniqueAttribution
 ...
 * @return {ol.control.Attribution} The attribution HTML.
 * @api stable
 */
ol.control.Attribution.uniqueAttributionList = [];
ol.control.Attribution.uniqueAttributionKey = {};

ol.control.Attribution.getUniqueAttribution = function(a, key) {
   if (!a) a = {html: ''};
   if (typeof(a)=='string') a = {html: a};
   if (key) {
      var attr = this.uniqueAttributionKey[key];
      if (!attr) {
         attr = this.uniqueAttributionKey[key] = new ol.control.Attribution(a);
      }
      return attr;
   } else {
      // Search existing
      ...
      // Create new one
      var u = new ol.control.Attribution(a);
